package quicksiiver.diamond_enhancements.client.tooltip;
...
public class ClientDiamondBundleTooltip implements ClientTooltipComponent {
	...
	public ClientDiamondBundleTooltip(final BundleContents contents) {
	...
	private static int getProgressBarFill(final Fraction weight) {
		return Mth.clamp(Mth.mulAndTruncate(weight.divideBy(Fraction.getFraction(4)), 94), 0, 94);
	}

	private static Identifier getProgressBarTexture(final Fraction weight) {
		return weight.compareTo(Fraction.getFraction(4)) >= 0 ? PROGRESSBAR_FULL_SPRITE : PROGRESSBAR_FILL_SPRITE;
	}
	...
	private static @Nullable Component getProgressBarFillText(final Fraction weight) {
		if (weight.compareTo(Fraction.ZERO) == 0) {
			...
			return weight.compareTo(Fraction.getFraction(4)) >= 0 ? BUNDLE_FULL_TEXT : null;
		}
	}
}
